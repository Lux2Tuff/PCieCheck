...
if (-not ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    Write-Warning "Relaunching script with Administrator privileges..."
    Start-Process powershell.exe -ArgumentList "-NoProfile -ExecutionPolicy Bypass -File `"$PSCommandPath`"" -Verb RunAs
    exit
}

# Define log file path
$LogFile = Join-Path $env:TEMP "PCIeDeviceCheck_$(Get-Date -Format 'yyyyMMdd_HHmmss').log"
...
$SuspiciousDMAPatterns = @(
    @{ VendorID = "10EE"; DeviceID = "0666"; Class = "FF00"; Description = "Xilinx FPGA (Screamer PCIe Squirrel stock)" },
    @{ VendorID = "1D0F"; DeviceID = "*"; Class = "FF00"; Description = "Amazon FPGA (Potential custom DMA firmware)" },
    ...
    @{ VendorID = "*"; DeviceID = "*"; Class = "FF00"; Description = "Unknown Device Class (Common for FPGA-based DMA)" }
)
...
function Test-SuspiciousDevice {
    ...
    foreach ($Pattern in $Patterns) {
        $VendorMatch = ($Pattern.VendorID -eq "*" -or ($VendorID -and $Pattern.VendorID -eq $VendorID))
        $DeviceMatch = ($Pattern.DeviceID -eq "*" -or ($DeviceID -and $Pattern.DeviceID -eq $DeviceID))
...
try {
    ...
    $PCIeDevices = Get-PnpDevice -PresentOnly -ErrorAction Stop | Where-Object { $_.InstanceId -like "PCI\*" } | 
    ...
    if (-not $PCIeDevices) {
        ...
        foreach ($Device in $PCIeDevices) {
            ...
            if ($NormalizedID) {
                ...
                # Retrieve Device Class Code from WMI
                $WmiDevice = Get-WmiObject -Class Win32_PnPEntity -Filter "PNPDeviceID='$InstanceId'" -ErrorAction SilentlyContinue
